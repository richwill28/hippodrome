...
public class TspSolver extends Thread {
	...
	static int new_tour(int prev_index, int move) {
		...
		synchronized (TourLock) {
			...
			curr.last = prev.last;
			curr.prefix_weight = prev.prefix_weight + weights[curr.prefix[curr.last]][move];
	...
	/*
	 * Add up min edges connecting all unconnected vertixes (AHU p. 331-335) At some
	 * point, we may want to use MST to get a lower bound. This bound should be
	 * higher (and therefore more accurate) but will take longer to compute.
	 */
	static int calc_bound(int curr_index) {
		...
		synchronized (TourLock) {
			...
			/*
			 * wt1: the value of the edge with the lowest weight from the node we're testing
			 * to another unconnected node. wt2: the value of the edge with the second
			 * lowest weight
			 */
			...
			/*
			 * Add up the lowest weights for edges connected to vertices not yet used or at
			 * the ends of the current tour, and divide by two. This could be tweaked quite
			 * a bit. For example: (1) Check to make sure that using an edge would not make
			 * it impossible for a vertex to have degree two. (2) Check to make sure that
			 * the edge doesn't give some vertex degree 3.
			 */
	...
	static boolean less_than(PrioQElement x, PrioQElement y) {
		return ((x.priority < y.priority) || (x.priority == y.priority) && (Tours[x.index].last > Tours[y.index].last));
	}
	 ...
	 * split_tour():
	 *
	 * Break current tour into subproblems, and stick them all in the priority queue
	...
	static void split_tour(int curr_ind) {
		...
		synchronized (TourLock) {
			...
			/*
			 * Create a tour and add it to the priority Q for each possible path that can be
			 * derived from the current path by adding a single node while staying under the
			...
			if (curr.last != (Tsp.TspSize - 1)) {
				...
				for (i = 0; i < Tsp.TspSize; i++) {
					/*
					 * Check: 1. Not already in tour 2. Edge from last entry to node in graph and 3.
					 * Weight of new partial tour is less than cur min.
					 */
					...
					if (t1 && t2 && t3) {
						...
						/*
						 * If it's shorter than the current best tour, or equal to the current best tour
						 * and we're reporting all min length tours, put it on the priority q.
						 */
						new_ = Tours[n_ind];
						...
						/*
						 * Bubble the entry up to the appropriate level to maintain the invariant that a
						 * parent is less than either of it's children.
						 */
						for (parent = pq >> 1, pptr = PrioQ[parent]; (pq > 1) && less_than(cptr,
								pptr); pq = parent, cptr = pptr, parent = pq >> 1, pptr = PrioQ[parent]) {
	 ...
	 * find_solvable_tour():
	 *
	 * Used by both the normal TSP program (called by get_tour()) and the RPC server
	 * (called by RPCserver()) to return the next solvable (sufficiently short)
	 * tour.
	 *
	 */
	static int find_solvable_tour() {
		...
		synchronized (TourLock) {
			...
			for (; PrioQLast != 0;) {
				...
				/*
				 * If we're within `NodesFromEnd' nodes of a complete tour, find minimum
				 * solutions recursively. Else, split the tour.
				 */
	 ...
	 *
	 * We're now supposed to recursively try all possible solutions starting with
	 * the current tour. We do this by copying the state to local variables (to
	 * avoid unneeded conflicts) and calling visit_nodes to do the actual recursive
	 * solution.
	 *
	 */
	...
	void visit_nodes(int from) {
		...
		for (i = 1; i < Tsp.TspSize; i++) {
			...
			if (PathLen == Tsp.TspSize) {
				...
				if ((last = weights[i][Tsp.StartNode]) != 0 && (CurDist += last) < MinTourLen) {
