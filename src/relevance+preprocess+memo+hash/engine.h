...
struct Engine {
  ...
  struct Memo {
    ...
    ankerl::unordered_dense::map<std::pair<Nonterminal, Transaction>,
                                 ankerl::unordered_dense::set<Event>>
        NRS; // neighbor reversed summary
    ankerl::unordered_dense::map<std::pair<Nonterminal, Transaction>,
                                 ankerl::unordered_dense::set<Event>>
        TDS; // trickle down summary
    ankerl::unordered_dense::map<std::pair<Nonterminal, Transaction>,
                                 ankerl::unordered_dense::set<Event>>
        TDRS; // trickle down reversed summary
    ankerl::unordered_dense::map<std::pair<Nonterminal, Transaction>,
                                 ankerl::unordered_dense::set<Event>>
    ...
    ankerl::unordered_dense::map<std::pair<Nonterminal, Transaction>,
                                 ankerl::unordered_dense::set<Event>>
        ATDS; // annotated trickle down summary
    ankerl::unordered_dense::map<std::pair<Nonterminal, Transaction>,
                                 ankerl::unordered_dense::set<Event>>
        ATDRS; // annotated trickle down reversed summary
    ankerl::unordered_dense::map<std::pair<Nonterminal, Transaction>,
                                 ankerl::unordered_dense::set<Event>>
        ACS; // annotated c summary
    ankerl::unordered_dense::map<std::pair<Nonterminal, Transaction>,
                                 ankerl::unordered_dense::set<Event>>
        ABRS; // annotated b reversed summary

    Memo()
        : NS{}, NRS{}, TDS{}, TDRS{}, TUS{}, TURS{}, ATDS{}, ATDRS{}, ACS{},
          ABRS{} {}
  };
  ...
  ankerl::unordered_dense::set<Event> compute_neighbor_summary_fixpoint(
    ...
    for (const auto &edge : core.aux_graph[B].edges) {
      ...
      std::pair<Nonterminal, Transaction> decor{A, Tj};
      if (memo.NS.contains(decor)) {
        neighbor_summary = memo.NS[decor];
  ...
  ankerl::unordered_dense::set<Event> compute_neighbor_summary(
    ...
    if (memo.NS.contains(decor)) {
      return memo.NS[decor];
    }

    ankerl::unordered_dense::set<Thread> avoid;

    memo.NS[decor] = compute_neighbor_summary_fixpoint(Ti, A, B, parent, avoid);
  ...
  ankerl::unordered_dense::set<Event> compute_trickle_down_summary(
    ...
    std::pair<Nonterminal, Transaction> d{A, Ti};

    if (memo.TDS.contains(d)) {
      return memo.TDS[d];
    }
  ...
  ankerl::unordered_dense::set<Event> annotate_trickle_down_summary(
      Transaction Ti, Nonterminal A, Nonterminal B,
      const ankerl::unordered_dense::set<Event> &summary) {

    std::pair<Nonterminal, Transaction> decor{A, Ti};

    if (memo.ATDS.contains(decor)) {
      return memo.ATDS[decor];
    }

    memo.ATDS[decor] = annotate_bottom_summary(B, summary);

    return memo.ATDS[decor];
  }

  ankerl::unordered_dense::set<Event>
  annotate_c_summary(Transaction Ti, Nonterminal A, Nonterminal B,
                     const ankerl::unordered_dense::set<Event> &summary) {

    std::pair<Nonterminal, Transaction> decor{A, Ti};

    if (memo.ACS.contains(decor)) {
      return memo.ACS[decor];
    }

    memo.ACS[decor] = annotate_bottom_summary(B, summary);
  ...
  ankerl::unordered_dense::set<Event> compute_trickle_up_summary_fixpoint(
    ...
    ankerl::unordered_dense::set<Event> annotated_trickle_down_summary =
        annotate_trickle_down_summary(Tj, A, B, trickle_down_summary);
    ...
    for (const Event &event : trickle_down_summary) {
      ...
      if (event.annotation == Annotation::up ||
        ...
        ankerl::unordered_dense::set<Event> res;
        std::pair<Nonterminal, Transaction> d{A, Tk};
        if (memo.TUS.contains(d)) {
          res = memo.TUS[d];
          avoid.insert(Tk.thread);
        } else {
          res = compute_trickle_up_summary_fixpoint(Ti, Tk, A, B, C, parent,
                                                    child, avoid);
        }
  ...
  void compute_summary(
    ...
    ankerl::unordered_dense::set<Event> annotated_trickle_down_summary =
        annotate_trickle_down_summary(Ti, A, B, trickle_down_summary);
    ...
    ankerl::unordered_dense::set<Event> annotated_c_summary =
        annotate_c_summary(Ti, A, B, c_summary);
    ...
    core.aux_graph[A].summary[Ti].insert(neighbor_summary.begin(),
                                         neighbor_summary.end());
    core.aux_graph[A].summary[Ti].insert(annotated_trickle_down_summary.begin(),
                                         annotated_trickle_down_summary.end());
    core.aux_graph[A].summary[Ti].insert(annotated_c_summary.begin(),
  ...
  compute_neighbor_reversed_summary_fixpoint(
    ...
    for (const auto &edge : core.aux_graph[C].edges) {
      ...
      std::pair<Nonterminal, Transaction> decor{A, Tj};
      if (memo.NRS.contains(decor)) {
        neighbor_reversed_summary = memo.NRS[decor];
        avoid.insert(Tj.thread);
      } else {
  ...
  ankerl::unordered_dense::set<Event> annotate_trickle_down_reversed_summary(
      Transaction Ti, Nonterminal A, Nonterminal C,
      const ankerl::unordered_dense::set<Event> &summary) {

    std::pair<Nonterminal, Transaction> decor{A, Ti};

    if (memo.ATDRS.contains(decor)) {
      return memo.ATDRS[decor];
    }

    memo.ATDRS[decor] = annotate_top_reversed_summary(C, summary);

    return memo.ATDRS[decor];
  }

  ankerl::unordered_dense::set<Event> annotate_b_reversed_summary(
      Transaction Ti, Nonterminal A, Nonterminal C,
      const ankerl::unordered_dense::set<Event> &summary) {

    std::pair<Nonterminal, Transaction> decor{A, Ti};

    if (memo.ABRS.contains(decor)) {
      return memo.ABRS[decor];
    }

    memo.ABRS[decor] = annotate_top_reversed_summary(C, summary);
  ...
  compute_trickle_up_reversed_summary_fixpoint(
    ...
    ankerl::unordered_dense::set<Event>
        annotated_trickle_down_reversed_summary =
            annotate_trickle_down_reversed_summary(
                Tj, A, C, trickle_down_reversed_summary);
    ...
    for (const Event &event : trickle_down_reversed_summary) {
      ...
      if (event.annotation == Annotation::down ||
        ...
        ankerl::unordered_dense::set<Event> res;
        std::pair<Nonterminal, Transaction> d{A, Tk};
        if (memo.TURS.contains(d)) {
          res = memo.TURS[d];
          avoid.insert(Tk.thread);
        } else {
          res = compute_trickle_up_reversed_summary_fixpoint(
  ...
  void compute_reversed_summary(
    ...
    ankerl::unordered_dense::set<Event>
        annotated_trickle_down_reversed_summary =
            annotate_trickle_down_reversed_summary(
                Ti, A, C, trickle_down_reversed_summary);
    ...
    ankerl::unordered_dense::set<Event> annotated_b_reversed_summary =
        annotate_b_reversed_summary(Ti, A, C, b_reversed_summary);
  ...
  bool compute_aux_graph_base_forward(
      ...
      ankerl::unordered_dense::map<Thread, std::vector<Transaction>>
          &transactions) {

    ankerl::unordered_dense::map<Thread, Transaction> alive;
    ...
    for (size_t i = 1; i < events.size(); i++) {
      ...
      if (event.type == EventType::read || event.type == EventType::write ||
        ...
        // For optimization
        ankerl::unordered_dense::set<Transaction> checked;
        ...
        if (alive.contains(event.thread)) {
          ...
          ankerl::unordered_dense::set<Event> updated_content;
          ...
          ankerl::unordered_dense::set<Transaction> immediate_preds;
          ankerl::unordered_dense::set<Transaction> immediate_succs;
  ...
  void compute_aux_graph_base(Nonterminal A, std::vector<Terminal> terminals) {
    ...
    ankerl::unordered_dense::map<Thread, std::vector<Transaction>> transactions;
    ...
    ankerl::unordered_dense::set<std::pair<Transaction, Transaction>>
        forward_edges = core.aux_graph[A].edges;
  ...
  void reduce_aux_graph_base(Nonterminal nonterminal) {
    ankerl::unordered_dense::set<std::tuple<Thread, EventType, Operand>>
        to_erase;
  ...
  void preprocess() {
    ...
    // ankerl::unordered_dense::map<Nonterminal,
    // ankerl::unordered_dense::set<Nonterminal>> parents;
    compute_parents();

    // ankerl::unordered_dense::map<Nonterminal,
    // ankerl::unordered_dense::set<Nonterminal>> ancestors;
    for (const Nonterminal &nonterminal : lowest_nonterminals) {
      compute_ancestor(nonterminal);
    }

    ankerl::unordered_dense::set<Operand> operands;
