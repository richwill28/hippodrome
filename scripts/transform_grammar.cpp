...
struct Grammar {
  std::unique_ptr<ankerl::unordered_dense::set<Nonterminal>> nonterminals;
  std::unique_ptr<ankerl::unordered_dense::set<Terminal>> terminals;
  std::unique_ptr<ankerl::unordered_dense::map<Nonterminal, std::deque<Symbol>>>
      rules;
  unsigned long long fresh_nonterminal_idx;

  Grammar()
      : nonterminals{std::make_unique<
            ankerl::unordered_dense::set<Nonterminal>>()},
        terminals{std::make_unique<ankerl::unordered_dense::set<Terminal>>()},
        rules{std::make_unique<
            ankerl::unordered_dense::map<Nonterminal, std::deque<Symbol>>>()},
  ...
  void transform_to_cnf() {
    ...
    // nonterminals.
    auto _rules = std::make_unique<
        ankerl::unordered_dense::map<Nonterminal, std::deque<Symbol>>>();
    *_rules = *rules;
    auto prod =
        std::make_unique<ankerl::unordered_dense::map<Terminal, Nonterminal>>();
  ...
  void
  print_aux(const ankerl::unordered_dense::map<Terminal, std::string> &event,
  ...
  void print(std::string map_path, Nonterminal nonterminal) {
    ankerl::unordered_dense::map<Terminal, std::string> event;
